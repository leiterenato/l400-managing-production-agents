...
> **Slide realization (updated 2026-07-06):** on stage these lies are shown **spatially** (failures clicked onto a real agent architecture — the talk's anchor diagram) and **temporally** (a demo → production-at-scale timeline for cold start / rot / cost), with a happiness **thermometer** as the throughline — *not* as a 3-column list. The 3-lies/Trap-Zero table above is the **conceptual** backbone; the staged form is in `docs/eval-agentes-fundamentos.md` §15.2.
...
- **Slide 2 — Problem (architecture as the stage):** open on a **realistic agent architecture** (main agent + refund/disputes sub-agents + tools + A2A to an outside fraud-check agent) for audience identification — this is also the **talk's cumulative anchor diagram** (Cases 2 & 3 reuse and attack it: `Payment system` → C2; `Customer database` + `Fraud-check/A2A` → C3). Click to reveal **spatial** failures as callouts (process: *wrong tool/order/loops* on the LLM, *poisoned memory* · money/PII: "look up customer" *leaks another account*, "issue refund" *> charge*); a **base timeline** (`Demo → Production`) carries the **temporal** pains (Cold Start · Eval Rot · Explosion) and ends on a red **Production** marker (*"all green, yet it breaks"*). The **conclusion** ("the green score lies" + the 2 questions) sits top-right and is **revealed last**. ~5 clicks; no thermometer. *Substrate note: OTel is deferred to the next slide. Full spec + Mermaid + speaker notes: `docs/eval-agentes-fundamentos.md` §15.2.*
- **Slide 3 — The turn (EDD) [depth spike A]** ⭐ *(introduces EDD as our proven technique):* the **star is the derivation** — contract → invariant → case, shown as a visual mechanic (not two static YAMLs). **"EDD = BDD for agents"; day zero**; the tie to Slide 2 (**policy P1 = the green `refund ≤ charge` invariant** — one object, three jobs); vs. User Simulation; the adversarial case = **eval as a security control** (exfiltration check from the I/O contract — bridge to Case 3). Resolves **cold start**. **[DEMO part 1]** *(full spec + Mermaid + speaker notes: `docs/eval-agentes-fundamentos.md` §15.4)*
- **Slide 4 — The flywheel + the "why" [depth spike B + end-to-end]:** the loop (seed arc kills cold start · refill arc from production kills rot) · the gate (Cloud Build, not native) · **the zoom into one failure** — output looks fine, trajectory shows the wrong tool, **Failure Clusters names it** (`generate_loss_clusters`, the "why") · the honest boundary (conformance vs. correctness) · the "isn't this just MLOps?" defense (seed derived from contract + reference-free judge) · scale proof · *(error budget as the bridge = one optional sentence only, not a beat)*. **[DEMO part 2]** · *hook into Case 2: "eval is pre-prod; under real load the dependencies don't cooperate."* *(full spec: `§15.5`)*
